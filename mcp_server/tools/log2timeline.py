...
from mcp_server.parsers.timeline_parser import parse_timeline_csv, summarize_timeline, filter_by_window
...
def register_log2timeline_tools(mcp, rag=None):
    ...
    def filter_timeline(
        ...
        if not csv_content.strip():
            return json.dumps({"error": "No events returned. Check time window or filter query.", "stderr": stderr})
