...
def _seed_rag(case_ioc_json: str = "") -> bool:
    ...
    try:
        ...
        if count == 0:
            # Offline-first seeding: Hunt Evil process baseline, ROCBA case IOCs, the
            # MITRE technique catalog, and a curated LOLBAS reference — all bundled, no
            # network. (A full MITRE ATT&CK download is attempted opportunistically.)
            try:
                from rag.ingest.knowledge_corpus import ingest_all_offline
                total = ingest_all_offline(kb)
                logger.info(f"Seeded offline knowledge corpus: {total} documents")
            except Exception as e:
                logger.warning(f"Offline corpus seeding failed: {e}")
            try:
                from rag.ingest.mitre_attack import ingest as ingest_mitre
                n = ingest_mitre()
                logger.info(f"Ingested {n} MITRE ATT&CK techniques (network)")
            except Exception as e:
                logger.warning(f"Full MITRE ATT&CK download skipped ({e}); offline corpus active")
